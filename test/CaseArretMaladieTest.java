...
class CaseArretMaladieTest {
	...
	CaseArretMaladie caseArret = new CaseArretMaladie();
	
	@Test
	/**
	 * Permet de tester la méthode actionCase de la classe
	 * dans le cas où le joueur1 est malade et souhaite payer pour se soigner donc perd 25€ et son nombre de tours en arrêt repasse à 1
	 */
	void actionCasePayerTest() {
		joueur1.setEstMalade(true);
		caseArret.setRep(true);
		joueur1.setToursEnArretMaladie(2);
		caseArret.actionCase(joueur1, plateau, null);
	...
	/**
	 * Permet de tester la méthode actionCase de la classe
	 * dans le cas où le joueur2 est malade et 	est forcé de payer puisque son nombre de tours en arrêt est > 2
	...
	void actionCaseImpayeTest() {
		...
		caseArret.actionCase(joueur2, plateau, null);
